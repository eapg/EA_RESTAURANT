...
from src.exceptions.exceptions import InvalidCredentialsException
...
def mock_oauth2_repository(oauth2_repository, client, user, client_user, client_scope):
...
class Oauth2RepositoryImplTest(unittest.TestCase):
    def setUp(self):
        ...
        self.mocked_password_encoder = mock.Mock()

        self.oauth2_repository = Oauth2RepositoryImpl(
            self.mocked_engine, self.mocked_password_encoder
        )

        self.test_client_id = "client1234"
        self.test_client_secret = "1234"
        self.test_username = "juperez"
        self.test_password = "1234abcd"
    ...
    def test_user_login_exception_wrong_password(
        self, mocked_build_user_access_token, mocked_build_user_refresh_token
    ):
        user = build_user()
        client = build_client()
        ...
        mocked_build_user_refresh_token.return_value = user_refresh_token
        self.oauth2_repository.password_encoder.validate_password.return_value = False
        mocked_oauth2_repository = mock_oauth2_repository(
            self.oauth2_repository, client, user, client_user, client_scope
        )
        ...
        self.assertRaises(
            ...
            mocked_oauth2_repository.login_user,
            self.test_client_id,
            self.test_client_secret,
            self.test_username,
            "wrong password",
    ...
    def test_client_login_exception_wrong_client_secret(
        self, mocked_build_client_access_token, mocked_build_client_refresh_token
    ):
        user = build_user()
        client = build_client()
        ...
        self.assertRaises(
            InvalidCredentialsException,
            mocked_oauth2_repository.login_client,
            self.test_client_id,
            "wrong client secret",
        )
    ...
    def test_user_login_exception_wrong_user(
        self, mocked_build_user_access_token, mocked_build_user_refresh_token
    ):
        user = build_user()
        client = build_client()
        ...
        mocked_oauth2_repository._get_user_by_username.side_effect = (
    ...
    def test_client_login_exception_wrong_client(
        self, mocked_build_client_access_token, mocked_build_client_refresh_token
    ):
        user = build_user()
        client = build_client()
        ...
        mocked_oauth2_repository._get_client_by_client_id.side_effect = (
            InvalidCredentialsException("invalid credentials")
        )
