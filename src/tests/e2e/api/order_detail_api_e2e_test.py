...
class OrderDetailApiE2ETest(BaseFlaskSetupTest):
    ...
    def test_add_order_detail_request(self):
        ...
        request = self.client.post("/order_details", json=json_order_detail)
        json_order_detail["id"] = 1
        self.assertEqual(request.get_json(), json_order_detail)

    def test_get_by_id_request_successfully(self):
        json_order_detail = build_order_detail()
        order_detail_1 = self.client.post("/order_details", json=json_order_detail)
        request = self.client.get("/order_details/1")
    ...
    def test_get_all_order_detail_request_successfully(self):
        ...
        order_detail_1 = self.client.post("/order_details", json=json_order_detail_1)
        order_detail_2 = self.client.post("/order_details", json=json_order_detail_2)
        order_detail_3 = self.client.post("/order_details", json=json_order_detail_3)

        request = self.client.get("/order_details")
    ...
    def test_updated_order_detail_request_successfully(self):
        json_order_detail = build_order_detail()
        self.client.post("/order_details", json=json_order_detail)
        order_detail_with_parameter_updated = {"quantity": 5, "updated_by": 2}
        request = self.client.put(
            "/order_details/1", json=order_detail_with_parameter_updated
        )
    ...
    def test_deleted_order_detail_request_successfully(self):
        json_order_detail = build_order_detail()
        self.client.post("/order_details", json=json_order_detail)
        request = self.client.delete("/order_details/1", json={"updated_by": 3})
        self.assertEqual(request.status_code, HttpStatus.OK.value)

    def test_get_by_ingredient_id_request_successfully(self):
        json_order_detail = build_order_detail()
        order_detail_1 = self.client.post("/order_details", json=json_order_detail)
        request = self.client.get("/order_details/by_order_id/1")
        self.assertEqual(request.get_json(), [order_detail_1.get_json()])
