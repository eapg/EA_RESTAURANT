...
class OrderApiE2ETest(BaseFlaskSetupTest):
    ...
    def test_add_order_request(self):
        ...
        request = self.client.post("/orders", json=json_order)
        json_order["id"] = 1
        self.assertEqual(request.get_json(), json_order)

    def test_get_by_id_request_successfully(self):
        json_order = build_order()
        order_1 = self.client.post("/orders", json=json_order)
        request = self.client.get("/orders/1")
    ...
    def test_get_all_order_request_successfully(self):
        ...
        order_1 = self.client.post("/orders", json=json_order_1)
        order_2 = self.client.post("/orders", json=json_order_2)
        order_3 = self.client.post("/orders", json=json_order_3)
    ...
    def test_updated_order_request_successfully(self):
        json_order = build_order()
        self.client.post("/orders", json=json_order)
        order_with_parameter_updated = {"status": "IN_PROCESS", "updated_by": 2}
        request = self.client.put("/orders/1", json=order_with_parameter_updated)
    ...
    def test_deleted_order_request_successfully(self):
        json_order = build_order()
        self.client.post("/orders", json=json_order)
        request = self.client.delete("/orders/1", json={"updated_by": 3})
    ...
    def test_orders_by_status_request_successfully(self):
        ...
        request = self.client.get(
            "/orders/by_order_status/IN_PROCESS?limit=10", json=json_order
        )
        self.assertEqual(request.get_json(), [order_1.get_json()])
    ...
    def test_get_order_ingredients_by_order_id(self):
        ...
        self.client.post("/orders", json=json_order)
        request = self.client.get("/orders/1/ingredients", json=json_order)
